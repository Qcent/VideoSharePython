...
def pil_to_webp_bytearray(pil_image, quality=85):
    # Convert PIL image to byte array
    byte_array = BytesIO()
    pil_image.save(byte_array, format='WebP', quality=quality)
...
def pil_to_jpeg_bytearray(pil_image, quality=85):
...
def jpeg_bytearray_to_pil(byte_array):
    # Convert byte array to PIL image
    pil_image = Image.open(BytesIO(byte_array))
